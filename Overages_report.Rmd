...
First taking a quick look at these data after removing "FUNZONE" and VMI to check that these have been excluded.
```{r, echo=FALSE}
...
Now we can calculate the percent of difference between Order Qty and Receipt Qty in order to plot a frequency distribution of the results. 
```{r, echo=TRUE}
Overages2 <- Overages2 %>% 
...
Finally, taking a look at the frequency distribution (of the percentage difference). 
```{r, echo=TRUE}
...
We can see that most PO's fall within the +- 5%. However there seems to be a heavy skew toward +5%.
...
We can see that there are still some large overages. Now let's take a look at the impact of those. 
```{r, echo=FALSE}
...
Below is the summary of the PO's with over 20% delta.
```{r, echo= FALSE}
Over_table_20 %>% 
  as.data.frame()

```
And the PO's over the 5% threshold.
```{r, echo=FALSE}
Over_table_5%>% 
  as.data.frame()
```
For good measure let's look at those negative values. First under negative 20%.
```{r, echo=FALSE}
Under_table_20%>% 
  as.data.frame()
```
...
Under_table_5%>% 
  as.data.frame()
```
Total Delta Units (including negative values) for the entire dataset:
```{r, echo=FALSE}
Overages_nodupe %>% 
  summarise(Summary_of_Delta = sum(Delta_Units)) 
```

Total Overages (Percent Delta > 0):
```{r, echo=FALSE}
...
Total Underage (Percent Delta < 0):
```{r, echo=FALSE}
...
```{r}
Overages_nodupe %>%   
  mutate(PAR_VENDOR_LEGAL_DESC = replace(PAR_VENDOR_LEGAL_DESC, PAR_VENDOR_LEGAL_DESC=="TETS N RAI", "TETS N RAI INTERNATIONAL"))%>%
